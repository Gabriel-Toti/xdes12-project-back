...
async function sendInactivityReminders() {
  try {
    ...
    logger.info(`Encontrados ${inactiveUsers.length} usuários inativos para lembrete`);
    ...
    for (const user of inactiveUsers) {
      try {
        ...
        logger.error(`Erro ao enviar lembrete para ${user.email}: ${error}`);
      }
    }
  } catch (error) {
    logger.error(`Erro ao processar lembretes de inatividade: ${error}`);
  }
}
...
async function sendPendingMatchesReminders() {
  try {
    ...
    logger.info(`Encontrados ${pendingMatches.length} matches pendentes para lembrete`);

    // Agrupar por propriedade para evitar múltiplos emails para o mesmo admin
    const matchesByProperty = new Map<string, typeof pendingMatches>();
    ...
    for (const [, matches] of matchesByProperty) {
      ...
      for (const admin of admins) {
        try {
          ...
          logger.info(`Lembrete de matches enviado para ${admin.users.email}`);
        } catch (error) {
          logger.error(`Erro ao enviar lembrete de matches para ${admin.users.email}: ${error}`);
        }
      }
    }
  } catch (error) {
    logger.error(`Erro ao processar lembretes de matches pendentes: ${error}`);
  }
}
...
async function sendWeeklyHighlights() {
  try {
    ...
    if (recentAnnouncements.length === 0) {
      logger.info('Nenhum anúncio novo esta semana para highlights');
      return;
    }
    ...
    logger.info(`Enviando highlights semanais para ${activeUsers.length} usuários ativos`);
    ...
    for (const user of activeUsers) {
      try {
        ...
        logger.info(`Highlights semanais enviados para ${user.email}`);
      } catch (error) {
        logger.error(`Erro ao enviar highlights para ${user.email}: ${error}`);
      }
    }
  } catch (error) {
    logger.error(`Erro ao processar highlights semanais: ${error}`);
  }
}

// Inicializar schedulers
export function initSchedulers() {
  logger.info('Inicializando schedulers...');

  // Lembretes de inatividade: toda segunda-feira às 10h
  cron.schedule('0 10 * * 1', () => {
    logger.info('Executando: Lembretes de inatividade');
    sendInactivityReminders();
  });

  // Lembretes de matches pendentes: a cada 3 dias às 14h
  cron.schedule('0 14 */3 * *', () => {
    logger.info('Executando: Lembretes de matches pendentes');
    sendPendingMatchesReminders();
  });

  // Highlights semanais: toda sexta-feira às 9h
  cron.schedule('0 9 * * 5', () => {
    logger.info('Executando: Highlights semanais');
