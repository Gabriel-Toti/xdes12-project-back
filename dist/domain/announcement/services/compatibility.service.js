...
exports.calculateUserPropertyCompatibility = calculateUserPropertyCompatibility;
exports.calculatePropertyCompatibility = calculatePropertyCompatibility;
 ...
 */
function calculateAttributeCompatibility(preferenceName, preferenceValue, ruleName, ruleValue) {
    // Se os nomes não correspondem, não há compatibilidade
    if (preferenceName !== ruleName) {
        return 0;
    }
    const attributeConfig = (0, attributes_1.getAttributeConfig)();
    const config = attributeConfig[preferenceName];
    if (!config) {
        return 0;
    }
 ...
 */
function calculateUserPropertyCompatibility(userPreferences, propertyRules) {
    ...
    for (const preference of userPreferences) {
        const matchingRule = propertyRules.find(rule => rule.attribute.name === preference.attribute.name);
        if (matchingRule) {
            // Calcula compatibilidade base (0 a 1)
            const baseCompatibility = calculateAttributeCompatibility(preference.attribute.name, preference.attribute.value, matchingRule.attribute.name, matchingRule.attribute.value);
 ...
 */
async function calculatePropertyCompatibility(userId, propertyId, prisma) {
    ...
    if (userId) {
        ...
        if (userPreferences.length > 0) {
            userCompatibility = calculateUserPropertyCompatibility(userPreferences.map(p => ({
    ...
    if (participants.length > 0) {
        ...
        for (const [_, preferences] of preferencesByUser) {
            const compatibility = calculateUserPropertyCompatibility(preferences.map(p => ({
                attribute: p.attribute,
                weight: p.weight
            })), rulesArray);
